...
$sk_allowed = ['img' => ['src' => []], 'a' => ['href' => [],'title' => []],'br' => [],'em' => [],'strong' => []];
...
function sk_adminHeader() {
  ...
  wp_print_styles( [ 'schreikasten_admin' ]);
}
...
function sk_header() {
  ...
  wp_register_script('schreikasten', $url.'/wp-content/plugins/schreikasten/schreikasten.js', ['soundmanager2', 'bax-universal', 'sack', 'htmlEncoder'], SK_HEADER_V);
  ...
  wp_print_scripts( [ 'schreikasten' ]);
  wp_print_styles( [ 'schreikasten' ]);
}
/**
...
function sk_dashboard() {
  global $wpdb;
  global $current_user;
  get_currentuserinfo(); // todo: replace with https://developer.wordpress.org/reference/functions/wp_get_current_user/
...
function sk_require_name_and_email() {
  ...
  // If we use the general configuration and the configuration say 'yes', or if we say 'yes'  
...
function sk_avatar($id, $size) {
  ...
  if (strlen($id) > 0 && $id !== 0 ) {
    ...
    $answer = get_avatar('',$size); // The anonymous
  }
  return $answer;
}
...
function sk_page_selector($size, $group = 1,$rand = false) {
  ...
  if ($groups <= $total_groups) {
    ...
    if ($groups - $group <= $group_limit) {
      // If the difference between the total groups 
      ...
      if ($group > $group_limit) {
        // If the group to show is greater than the group limit. 
...
function skLegacy_updateWidget() {
  ...
  foreach ( (array) $sidebars_widgets as $index => $sidebar ) {
    if ( is_array($sidebar) ) {
      ...
      // If we found the widget, move all the widgets after the one where we are searching, then add the new widgets
      if ($changed) {
...
function sk_activate() {
  ...
  switch ($db_version) {
    ...
    default: // It's a fresh installation, create the table.
      ...
      // Widget options
      $options = ['title' => __('Schreikasten', 'sk'), 'registered' => false, 'avatar' => true, 'layout' => SK_LAYOUT_SHOUTBOX, 'alert_about_emails' => true, 'items' => '5', 'refresh' => 0, 'bl_days' => '7', 'bl_maxpending' => '2', 'announce' => '1', 'requiremail' => '1', 'maxchars' => '225', 'rss' => false, 'moderation' => SK_MODERATION_CONFIG, 'date_format' => 'l, M j. Y h:i A'];
  ...
  // Widget options
  $options = ['title' => __('Schreikasten', 'sk'), 'registered' => false, 'avatar' => true, 'layout' => SK_LAYOUT_SHOUTBOX, 'alert_about_emails' => true, 'items' => '5', 'refresh' => 0, 'bl_days' => '7', 'bl_maxpending' => '2', 'announce' => '1', 'requiremail' => '1', 'maxchars' => '225', 'rss' => false, 'moderation' => SK_MODERATION_CONFIG, 'delete_type' => 1, 'delete_num' => 0, 'maxperday' => 0];
  add_option('sk_options', $options);
...
function sk_inform($id) {
  ...
  if ($comment->status === SK_HAM) {
    if ($announce === SK_ANNOUNCE_YES || ($announce === SK_ANNOUNCE_CONFIG && get_option('comments_notify'))) {
      ...
      if ($comment->email !== '') {
        $notify_message .= sprintf( __('E-mail : %s', 'sk'), sanitize_email($comment->email) ) . "\r\n";
      }
...
function sk_add_comment($alias, $email, $text, $ip, $for) {
  ...
  $user_can_send = true;
  if (sk_only_registered_users() && $user_id <= 0) {
    $user_can_send = false;
  }
...
function sk_allowed_messages($pc = false) {
  ...
  if (!current_user_can( SK_CAP )) {
    ...
    if ($allowed_perday !== -1 || $allowed_blocked !== -1) {
      if ($allowed_perday === -1) {
        $answer = $allowed_blocked;
      }
      if ($allowed_blocked === -1) {
        $answer = $allowed_perday;
      }
    }
    if ($allowed_perday !== -1 && $allowed_blocked !== -1) {
      if ($allowed_perday > $allowed_blocked) {
        $answer = $allowed_blocked;
      } else {
        $answer = $allowed_perday;
      }
...
function sk_canReply() {
  ...
  if ($options['layout'] === SK_LAYOUT_BOARD || $options['layout'] === SK_LAYOUT_CHAT || ($options['layout'] === SK_LAYOUT_QA && $sk_canmannage)) {
    $answer = true;
  }
...
function sk_format_comment($comment,$sending = false,$rand = false,$hide = false) {
  ...
  $sk_canmannage = current_user_can(SK_CAP);
...
function sk_config() {
  ...
  $db_version = get_option('sk_db_version');
  if (!$db_version || $db_version < SK_DB_VERSION) {
    sk_activate();
  }
  ...
  if (isset($_POST['sk-submit']) && $_POST['sk-submit'] ) {
    ...
    if (!is_numeric($options['maxchars'])) {
      $options['maxchars'] = 225;
    }
    
    $options['items'] = $_POST['sk_items'];
    if (!is_numeric($options['items'])) {
      $options['items'] = 5;
    }
...
function sk_manage() {
  ...
  if (isset($_POST['mode_x'])) {
    $mode_x = $_POST['mode_x'];
  }
  if (!$mode_x) {
    if (isset($_GET['mode_x'])) {
      $mode_x = $_GET['mode_x'];
    }
  }
  if (isset($_GET['mode'])) {
  ...
  if (isset($_POST['doaction2']) && $_POST['doaction2'] !== '') {
    $doaction = $_POST['action2'];
  }
  ...
  switch ($mode_x) {
    ...
    case 'set_spam_x': // set as spam
      ...
      if (sk_mark_as_spam($id)) {
        array_push($messages, __("Status changed",'sk'));
      }
      break;
    case 'delete_x': // delete the comment
      $id = $_GET['id'];
      if (sk_delete_comment($id)) {
        array_push($messages, __("Comment deleted",'sk'));
      }
      break;
    case 'forever_lock_x': // block forever
      $id = $_GET['id'];

      if (sk_lock_forever($id)) {
        array_push($messages, __("PC locked forever",'sk'));
      }
      break;
    case 'unlock_x': // unlock the PC
      ...
      if (sk_unlock($id)) {
        array_push($messages, __("PC unlocked",'sk'));
      }
      break;
...
function sk_codeShoutbox($size = false) {
  ...
  $length = __("The length of the message is bigger than the allowed size.", "sk");
  ...
  $answer = file_get_contents(ABSPATH . 'wp-content/plugins/schreikasten/templates/sk_widget.php');
  ...
  $answer = str_replace('%length%', $length, $answer);
...
*
*/
// check version. only WP 2.8 and up supports class multi widget system
if ((float)$wp_version >= 2.8) { // The new widget system
  class SkWidget extends WP_Widget {
    function __construct() {
      $control_ops = [ 'width' => 420, 'height' => 280 ];
      parent::WP_Widget('sk', 'Schreikasten', ['description' => __('Add a Schreikasten Shoutbox to the sidebar.', 'sk') ], $control_ops);
    }
    ...
    function form($instance) {
      $default = ['title' => 'Schreikasten', 'items' => '5','rss' => '0'];
